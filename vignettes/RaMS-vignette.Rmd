---
title: "Intro to RaMS"
date: "Updated 12/8/2020"
output: pdf_document
---

```{r setup, include = FALSE}
# RUN OUTSIDE OF PROJECT OR KNITR WILL EAT YOUR OUTPUT
# https://stackoverflow.com/questions/62432329/in-rstudio-where-is-knitr-markdown-output-located
...
data_files <- list.files(msdata_dir, pattern = "HL.*mzML", full.names = TRUE)
...
```{r MS2demo}
homarine_mass <- 137.047678+1.007276

homarine_MS2 <- DDA_data$MS2[premz%between%pmppm(homarine_mass, 5)]
...
clean_MS2 <- file_data$MS2 %>% 
  filter(premz%between%pmppm(homarine_mass, 10)) %>%
  group_by(rt) %>%
  arrange(desc(int)) %>%
...
file_data$MS1 %>% 
  filter(mz%between%pmppm(homarine_mass, 10)) %>%
...
homarine_neutral_loss <- homarine_mass - homarine_frag_mz
...
masses_of_interest <- c(adenine=136.06232, valine=118.0865)
...
```{r rtrangedemo}
small_data <- grabMSdata(data_files, grab_what = c("MS1", "MS2"), rtrange = c(340, 410))
